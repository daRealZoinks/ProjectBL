...
public class LateralTiltController : MonoBehaviour
{
    ...
    [SerializeField, Range(0, 10f)] private float angle = 3f;
    [Tooltip("The speed of the tilt when the player is moving")]
    [SerializeField, Range(0, 15f)] private float speed = 8f;
    ...
    private void Update()
    {
        if (lateralTiltEnabled)
        {
            ...
            targetRotation.x = transform.localRotation.eulerAngles.x;
            targetRotation.y = transform.localRotation.eulerAngles.y;
